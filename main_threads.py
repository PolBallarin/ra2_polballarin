...
OUTPUT_DIR = "raw"
PAGE_SIZE = 500
PAGE_SIZE_SMALL = 100     # Para tags y series (límite API = 300)
DELAY = 0.2

ENDPOINTS = ["tags", "events", "markets", "series"]
SMALL_ENDPOINTS = {"tags", "series"}
...
def fetch_all_records(endpoint: str) -> list:
    all_records = []
    offset = 0
    page_size = PAGE_SIZE_SMALL if endpoint in SMALL_ENDPOINTS else PAGE_SIZE

    print(f"[{endpoint.upper()}] Iniciando extracción (page_size={page_size})...")
    ...
    while True:
        ...
        try:
            ...
            print(f"[{endpoint.upper()}] ERROR offset={offset}: {e}")
            break
        ...
        print(f"[{endpoint.upper()}] {len(all_records)} registros (offset={offset})")
    ...
    print(f"[{endpoint.upper()}] TOTAL: {len(all_records)} registros")
    return all_records


def process_endpoint(endpoint: str) -> tuple:
    """Descarga y guarda un endpoint completo. Retorna (endpoint, data)."""
...
def save_to_delta(data: list, endpoint: str):
    ...
    write_deltalake(output_path, df, mode="overwrite", schema_mode="overwrite")
...
def main():
    ...
    # 4 threads, uno por endpoint, en paralelo
    with ThreadPoolExecutor(max_workers=4) as executor:
        results = list(executor.map(process_endpoint, ENDPOINTS))

    data_dict = {endpoint: data for endpoint, data in results}
